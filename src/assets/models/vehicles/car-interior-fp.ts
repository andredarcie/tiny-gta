...
function bar(mat: THREE.Material,thick: number,ax: number,ay: number,az: number,bx: number,by: number,bz: number): THREE.Mesh{
...
function vent(x: number,y: number,z: number): THREE.Group{
...
function makeGauge(x: number,withNeedle: boolean): {group: THREE.Group; needle: THREE.Group|null}{
  ...
  let needle: THREE.Group|null=null;
  if(withNeedle){
...
function addWheelArms(wheelSpin: THREE.Group): void{
...
export function buildCarInteriorFp(): THREE.Group{
  const g=new THREE.Group();

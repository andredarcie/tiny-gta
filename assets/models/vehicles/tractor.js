...
function box(mat,w,h,d,x,y,z,rx=0,ry=0,rz=0,cast=true){
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);
  m.position.set(x,y,z);m.rotation.set(rx,ry,rz);m.castShadow=cast;return m;
}
function cyl(mat,rt,rb,h,seg,x,y,z,rx=0,ry=0,rz=0,cast=true){
...
export function buildTractor(){
  ...
  g.add(box(GREEN_D,.74,.1,1.48, 0,.66,1.02,false));   // lower hood trim
  ...
  g.add(box(METAL,.36,.04,.46, -.4,.5,.46,false));     // left footplate
  g.add(box(METAL,.36,.04,.46,  .4,.5,.46,false));     // right footplate
  ...
  g.add(box(DARK,1.2,.12,.14, 0,.43,1.24,false));      // front axle beam
...
export function makeTractor(){const g=buildTractor();scene.add(g);return g;}

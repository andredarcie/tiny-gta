...
// uma poça (bloodstain) instanciada no mundo
interface Pool{
  id: string;
  name: string;
  money: number;
...
// shape de uma stain crua vinda do backend
interface RawStain{id?: string; name?: string; money?: number; x?: number; z?: number; own?: boolean;}

// id -> {id,name,money,x,z,y,g,phase,label,claiming}
const pools = new Map<string, Pool>();
let lastFetch = -999, fetching = false;

const fmtMoney = (n: unknown): string => '$' + Math.max(0, Math.floor(Number(n) || 0)).toLocaleString('en-US');

// ----- criação/remoção das poças no mundo ----------------------------------
function addPool(id: string, s: RawStain): void {
...
function disposeLabel(p: Pool): void {
  if (!p.label) return;
  scene.remove(p.label);
  (p.label.material.map as THREE.Texture | null)?.dispose?.();
...
function ensureLabel(p: Pool): void {
  if (p.label) return;
...
refs.dropDeathPool = function (x: number, z: number, money: number): void {
...
async function refreshBloodstains(): Promise<void> {
  if (fetching) return;
  fetching = true;
  const pid = getPlayerId();
  try {
    const r = await fetch(API + '/api/bloodstains?limit=120' + (pid ? '&pid=' + encodeURIComponent(pid) : ''));
    const data = await r.json() as {stains?: RawStain[]};
...
async function tryClaim(p: Pool): Promise<void> {
  p.claiming = true;
  ...
  try {
    ...
    const data = await r.json().catch(() => ({})) as {claimed?: boolean; money?: number; name?: string};
...
// ----- loop -----------------------------------------------------------------
export function updateBloodstains(dt: number): void {

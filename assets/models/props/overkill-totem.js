...
export function makeOverkillTotem(){
  ...
  cranium.scale.set(.95,1.1,.82);cranium.position.y=.18;cranium.castShadow=true;skull.add(cranium);
  const jaw=new THREE.Mesh(new THREE.BoxGeometry(.68,.34,.42),bone);
  jaw.position.set(0,-.35,-.02);jaw.castShadow=true;skull.add(jaw);
  for(const side of[-1,1]){
    ...
    cheek.scale.set(1,.72,.5);cheek.position.set(side*.27,-.1,-.32);cheek.castShadow=true;skull.add(cheek);
  }
  ...
  for(let i=0;i<5;i++){
    ...
    tooth.position.set((i-2)*.09,-.58,-.26);tooth.castShadow=true;skull.add(tooth);
  }

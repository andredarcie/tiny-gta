...
export function addShallowsAndWaves(groundHalf: number,beach: number): {m: THREE.Mesh; ph: number; spd: number; amp: number}[]{
  ...
  const waves: {m: THREE.Mesh; ph: number; spd: number; amp: number}[]=[];
  for(let k=0;k<3;k++){

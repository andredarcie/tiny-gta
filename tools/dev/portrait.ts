...
// Loaded by /portrait.html and screenshotted by test/portrait.spec.ts.
...
import {buildToonPlayer} from '/src/assets/models/characters/pedestrian.ts';

// Seed Math.random so the player ped is identical every run (stable comparison).
function mulberry32(a: number){return function(){a|=0;a=a+0x6D2B79F5|0;let t=Math.imul(a^a>>>15,1|a);t=t+Math.imul(t^t>>>7,61|t)^t;return((t^t>>>14)>>>0)/4294967296;};}
function seeded<T>(seed: number,fn: ()=>T): T{const o=Math.random;Math.random=mulberry32(seed);try{return fn();}finally{Math.random=o;}}

function renderPed(view: string,w: number,h: number): HTMLCanvasElement{
...
function loadImg(src: string): Promise<HTMLImageElement>{return new Promise((res,rej)=>{const i=new Image();i.onload=()=>res(i);i.onerror=()=>rej(new Error('img '+src));i.src=src;});}
function drawContain(ctx: CanvasRenderingContext2D,img: HTMLImageElement,x: number,y: number,w: number,h: number){
  const s=Math.min(w/img.width,h/img.height),dw=img.width*s,dh=img.height*s;
  ctx.drawImage(img,x+(w-dw)/2,y+(h-dh)/2,dw,dh);
}
function label(ctx: CanvasRenderingContext2D,t: string,x: number,y: number){
...
(async()=>{
 try{
  ...
  const cmp=document.getElementById('cmp') as HTMLCanvasElement,ctx=cmp.getContext('2d')!;
  ...
  console.error('[portrait] FAILED',String((e as Error)?.stack||e));
  (window as any).__err=String((e as Error)?.stack||e);
 }
})();

...
import {N,nodeX,HALF,CELL,ROAD,SIDE,irand,pick,clamp,wrapA,groundHeight} from './constants.js';
...
let timeLeft=0;
let destroyed=0;
let targets=[]; // [{g, ring, beacon, alive, gold, moving, axis, dir, speed, nextNode, panic}]
let crates=[];  // [{g, type, base, t, alive}]
...
const snapNode=v=>nodeX(clamp(Math.round((v+HALF)/CELL),0,N));
// Car yaw for an axis-aligned heading (engine forward = (sin h, cos h)).
const axisYaw=(ax,dr)=>ax==='x'?(dr>0?Math.PI/2:-Math.PI/2):(dr>0?0:Math.PI);
function turnTo(g,yaw,dt){ g.rotation.y+=wrapA(yaw-g.rotation.y)*Math.min(1,10*dt); }
...
function spawnTarget(){
  ...
  const t={g,ring,beacon,alive:true,gold,moving,axis:'x',dir:1,speed:0,nextNode:0,panic:0};

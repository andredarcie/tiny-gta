...
import {N,ROAD,BLOCK,SIDE,GROUND,BEACH,RURAL_X0,RURAL_X1,RURAL_HALF,RURAL_GAP,
  MOUNT_X,MOUNT_R,TOWN_CX,nodeX,groundHeight,ruralRoadPath} from './constants.js';
import {makeRng} from './rng.js';

// Reserved city blocks (mirror CLUB/GYM/HOSP/PRISON/GUNSHOP/WORKSHOP _I/_J).
const RESERVED=[[0,3],[7,1],[6,6],[2,2],[1,5],[5,2]];
...
const FARMHOUSES=[[212,-12],[236,10],[258,12],[282,-12],[302,10],[222,74],[310,-58]]
...
const isReserved=(i,j)=>RESERVED.some(([a,b])=>a===i&&b===j);
...
export function generateWorldSpec(seed=1337){
  ...
  const beachPalms=[],beachUmbrellas=[],beachChairs=[],beachRocks=[];
  ...
  const details=[];                                           // {t:'mushroom'|'log',x,z}
  let nd=0,dg=0;
  while(nd<80&&dg++<1600){
    const[gx,gz,rx,rz]=groves[irand(0,groves.length-1)];

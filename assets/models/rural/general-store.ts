...
export const genStoreFx: {
  keeper:{g:THREE.Group;t:number}|null,exitArrow:THREE.Object3D|null,facade:THREE.Group|null,facadeArrow:THREE.Object3D|null,
  footprint:{x0:number;x1:number;z0:number;z1:number}|null,seedCrates:THREE.Group[]
}={keeper:null,exitArrow:null,facade:null,facadeArrow:null,
...
let chalkTex: THREE.CanvasTexture|null=null;
...
let hangTex: THREE.CanvasTexture|null=null;
function hangTexture(): THREE.CanvasTexture{
...
// ---- small porch props ----
function extBarrel(): THREE.Group{
...
function extCrate(produce: boolean): THREE.Group{
  const g=new THREE.Group();
...
function extChalkSign(): THREE.Group{
  const g=new THREE.Group();
...
function labelPanel(lines: string,w: number,h: number,{bg='#2a1a0e',fg='#ffe7b0',sub='#f4c542'}: {bg?: string; fg?: string; sub?: string}={}): THREE.Mesh{
...
function addBox(x: number,y: number,z: number,w: number,h: number,d: number,mat: THREE.Material): THREE.Mesh{
...
function addShelfUnit(x: number,z: number,rotY: number,len: number): void{
...
function addSacks(x: number,z: number): void{
...
function makeSeedDisplay(s: typeof STRAINS[number]): THREE.Group{
  const g=new THREE.Group();
...
function addRegister(x: number,y: number,z: number,rotY: number): void{
...
function addStove(x: number,z: number,ceilY: number): void{
  const g=new THREE.Group();g.position.set(x,0,z);
...
function addBarrel(x: number,z: number): void{
  const g=new THREE.Group();g.position.set(x,0,z);
...
function addCandyJar(x: number,y: number,z: number,candyCol: number): void{
...
export function addGeneralStore(solids: {x0:number;x1:number;z0:number;z1:number;h:number}[]): void{

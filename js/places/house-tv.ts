...
function focusFrame(){
  if(!active)return;
  const frame=$('house-tv-frame') as HTMLIFrameElement|null;
  if(!frame)return;
...
function onFrameLoad(){
  focusFrame();
  try{
    const w=($('house-tv-frame') as HTMLIFrameElement).contentWindow as (Window&{__tvEscBound?:boolean})|null;
    if(w&&!w.__tvEscBound){
      w.__tvEscBound=true;
...
$('house-tv-exit')?.addEventListener('pointerdown',(e:Event)=>{
  e.preventDefault();
  e.stopPropagation();
  closeHouseTv();
});

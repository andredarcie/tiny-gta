...
const truck={g:makeFireTruck(),heading:0,speed:0,name:'FIRE TRUCK',firetruck:true};
...
let fire=null;       // {x,z,gy,group,ring,beacon,car,hp,maxHp,startT}
...
const smokePool=[],smoke=[];   // {m,vy,grow,t,life}
const emberPool=[],embers=[];  // {m,vx,vy,vz,t,life,s0}
const dropPool=[],drops=[];    // {m,vx,vy,vz,t,life,s0}
...
function spawnDrop(ox,oy,oz,tx,ty,tz){
...
(refs.carEnterLabels||(refs.carEnterLabels=[])).push(c=>
  c===truck?{label:'FIRE',prompt:'START FIRE BRIGADE',enabled:true}:null);
...
function updateFire(dt){
  ...
  const dist=Math.hypot(tp.x-fire.x,tp.z-fire.z);
  const inRange=dist<SPRAY_RANGE;

  // mira do canhão: aponta pro fogo quando perto, volta pra frente quando longe
  if(cannon){
    const want=inRange?Math.atan2(fire.x-tp.x,fire.z-tp.z)-truck.g.rotation.y:0;
  ...
  const frac=fire.hp/fire.maxHp;
  const top=fire.gy+2.2*blazeScale(frac);
  smokeAcc+=dt;
  const smokeEvery=1/(3+frac*5);        // fogo forte fumega mais
  while(smokeAcc>=smokeEvery){smokeAcc-=smokeEvery;spawnSmoke(fire.x,top,fire.z,true);}
  emberAcc+=dt;
  const emberEvery=1/(5+frac*7);
  while(emberAcc>=emberEvery){emberAcc-=emberEvery;
    spawnEmber(fire.x+rand(-.8,.8),fire.gy+.6+rand(0,1),fire.z+rand(-.8,.8));}

  if(inRange){
    setSpraying(true);
    fire.hp-=DOUSE_RATE*dt;
    ...
    if(nozzle){
      ...
      while(dropAcc>=dropEvery){dropAcc-=dropEvery;
        spawnDrop(_noz.x,_noz.y,_noz.z,fire.x,fire.gy+1,fire.z);}
    }
    // vapor branco subindo de onde a água bate (o fogo "geme")
    if(Math.random()<dt*16)spawnSmoke(fire.x,fire.gy+.7,fire.z,false);
    if(fire.hp<=0){extinguishFire();return;}
  }else{
    setSpraying(false);
    if(fire.hp<fire.maxHp)fire.hp=Math.min(fire.maxHp,fire.hp+REGEN_RATE*dt);
  }
  // encolhe/cresce o fogo conforme a vida
  fire.group.scale.setScalar(blazeScale(fire.hp/fire.maxHp));
}
...
export function updateFirefighter(dt){

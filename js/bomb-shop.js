...
let armedCar=null;
let beacon=null;     // mesh do farol vermelho que mostramos sobre o carro armado
...
(refs.zoneActions||(refs.zoneActions=[])).push(()=>{
  ...
  if(armedCar===cur)return{label:'ARMED',prompt:'BOMB ALREADY ARMED',enabled:false};
  // veículo único/de missão: mostra o prompt mas desabilitado (não corrompe).
  if(isSpecialVehicle(cur))
    return{label:'BOMB',prompt:'CANT RIG THIS VEHICLE',enabled:false};
...
export function updateBombShop(dt){
  t+=dt;
  if(!armedCar)return;

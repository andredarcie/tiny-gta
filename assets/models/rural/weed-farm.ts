...
let priceTex: THREE.CanvasTexture|null=null;
function priceTexture(): THREE.CanvasTexture {
...
export function makeWeedPlant(s=1,frosty=false,leafColor: THREE.ColorRepresentation|null=null): THREE.Group {
...
export function makeBud(s=1): THREE.Mesh {
...
export function makeBucket(withWater=true): THREE.Group {
  const g=new THREE.Group();
...
export function makeWaterDrop(): THREE.Mesh {return new THREE.Mesh(dropGeo,dropM);}

// ---- compound wall (a run along +x, centred), with base, coping, grooves
// and evenly-spaced pilasters. `len` is the run length. ----
function makeWall(len: number): THREE.Group {
  const g=new THREE.Group();
...
function makeGatePier(): THREE.Group {
  const g=new THREE.Group();
  const H=WALL_H+.9;
...
function makeRollingGate(): THREE.Group {
  const g=new THREE.Group();
...
function makeShack(): THREE.Group {
  const g=new THREE.Group();
...
function makeStandpipe(): THREE.Group {
  const g=new THREE.Group();
...
function makeSaleTable(): THREE.Group {
  const g=new THREE.Group();
...
export function makeCrate(s=1): THREE.Group {
  const g=new THREE.Group();
...
function makeEmptyBed(): THREE.Group {
  const g=new THREE.Group();
...
function makeSackStack(): THREE.Group {
  const g=new THREE.Group();
...
function makeFloodlight(yaw: number): THREE.Group {
  const g=new THREE.Group();
...
function addGrowLighting(g: THREE.Group): void {
...
export function addWeedFarm(solids: {x0:number;x1:number;z0:number;z1:number;h:number}[]): void {
  const g=build();

...
// A single armed gang member on the street.
...
function spawnMember(gang:Gang){
  ...
  const m:GangMember={g:makePed(gang.color,gang.pants),gang,state:'walk',vel:new THREE.Vector3(),
...
export function killGangPed(m:GangMember,dir:THREE.Vector3){
  ...
  gangCasualty(m);
  addWanted(.4,null as unknown as string,'ped_shot');
}

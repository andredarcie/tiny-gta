...
import {getInventory,equipWeaponById} from './weapons.js';
import {paintWeaponGlyph} from './weapon-icon.js';

const $=id=>document.getElementById(id);
let root=null,canvas=null,ctx=null;
let items=[];            // snapshot do inventário no momento da abertura
...
const segAngle=i=>-Math.PI/2+i*2*Math.PI/items.length; // setor 0 no topo, horário
function angDiff(a,b){let d=(a-b)%(2*Math.PI);if(d>Math.PI)d-=2*Math.PI;if(d<-Math.PI)d+=2*Math.PI;return d;}
function selFromAngle(theta){
  let best=0,bd=1e9;
...
function pointAimAt(i){const a=segAngle(i);aim.x=Math.cos(a);aim.y=Math.sin(a);}

// PC: delta do mouse (pointer lock) gira a mira da roda.
export function wheelPointerDelta(dx,dy){
  if(!state.wheelOpen)return;
...
export function wheelScroll(dir){
...
export function updateWeaponWheel(dt){
...
export function setupWheel(){
  root=$('weapon-wheel');canvas=$('ww-canvas');
  ...
  let touching=false;
  const onDown=e=>{if(!state.wheelOpen)return;e.preventDefault();touching=true;handleTouch(e.clientX,e.clientY);};
  const onMove=e=>{if(!state.wheelOpen||!touching)return;handleTouch(e.clientX,e.clientY);};
  const onUp=e=>{if(!state.wheelOpen||!touching)return;e.preventDefault();touching=false;closeWheel(!touchInDead);};

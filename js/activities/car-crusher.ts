...
let car: any=null;        // carro sendo esmagado (capturado em startCrush)
let scrap: any=null;      // refer. ao .g do carro achatado durante o "hold"
...
interface Debris{m:THREE.Mesh;dust:boolean;vx:number;vy:number;vz:number;t:number;life:number;spin:number;grow?:number;}
const debris: Debris[]=[];        // ativos: {m,dust,vx,vy,vz,t,life,spin,grow}
const shardPool: THREE.Mesh[]=[];     // cacos (caixinhas) reaproveitáveis (já na cena)
const dustPool: THREE.Mesh[]=[];      // baforadas (planos) reaproveitáveis (já na cena)
...
function spawnImpactFX(x: number,y: number,z: number){
  for(let i=0;i<14;i++){
    const m=takeShard();
  ...
  for(let i=0;i<5;i++){
    ...
    (m.material as THREE.MeshBasicMaterial).opacity=.7;
...
function updateDebris(dt: number){
  for(let i=debris.length-1;i>=0;i--){
    ...
    if(k>=1){
      d.m.visible=false;
      if(d.dust){(d.m.material as THREE.MeshBasicMaterial).opacity=.7;dustPool.push(d.m);}
    ...
    if(d.dust){
      ...
      (d.m.material as THREE.MeshBasicMaterial).opacity=.7*(1-k);
    }else{
...
export function updateCarCrusher(dt: number){

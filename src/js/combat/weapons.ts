...
import {blip,thud,gunshot,pistolShot} from '@/audio/audio.ts';
...
// A 3D ray reused by the aim helpers (origin/dir are shared scratch Vector3s).
interface Ray{origin: THREE.Vector3;dir: THREE.Vector3;}
// Result of findWeaponHit — `target`/`arr` come from loose .js registries.
interface WeaponHit{kind: string;d: number;target: any;arr?: any;}
// In-flight / transient effect records kept in per-frame pools.
interface MissileFx{g: THREE.Object3D;dir: THREE.Vector3;dist: number;range: number;}
interface ExplosionFx{g: THREE.Object3D;t: number;}
interface TracerFx{line: THREE.Object3D;t: number;}
interface ImpactFx{ring: THREE.Object3D;t: number;}
interface BulletFx{g: THREE.Object3D;dir: THREE.Vector3;prev: THREE.Vector3;speed: number;life: number;dist: number;range: number;damage: number;}
interface ThrownFx{g: THREE.Object3D;vel: THREE.Vector3;kind: string;fuse: number;spin: THREE.Vector3;life: number;}
interface FlameJetFx{g: THREE.Object3D;t: number;}
interface FirePoolFx{g: THREE.Object3D;t: number;life: number;nextTick: number;radius: number;}
interface MeleeTrailFx{m: THREE.Mesh;t: number;life: number;}
interface PlantedBomb{g: THREE.Object3D;}
// In-flight melee swing animation state.
interface MeleeAnim{kind: string;side: number;t: number;dur: number;hitAt: number;hitDone: boolean;range: number;knock: number;lethal: boolean;}
// Snapshot of the whole arsenal for session-temporary grants (rampage/training).
interface ArsenalSnapshot{owned: Weapon[];cur: Weapon;weapons: {w: Weapon;ammo: number;last: number}[];}
// Bullet spawn options.
interface BulletOpts{speed?: number;range?: number;damage?: number;}

const weaponPickups: THREE.Object3D[]=[];
function makeWeaponPickup(x: number,z: number){
...
export function selectWeaponSlot(slot: number){
...
export function equipWeaponById(id: string|undefined){
...
const tracers: TracerFx[]=[];
const impacts: ImpactFx[]=[];
const bullets: BulletFx[]=[];
const thrown: ThrownFx[]=[];     // granadas/molotovs em arco
const flameJets: FlameJetFx[]=[];  // jatos do lança-chamas (efeito visual, vida curta)
const firePools: FirePoolFx[]=[];  // poças de fogo (molotov / carga do detonador)
let plantedBomb: PlantedBomb|null=null; // carga do detonador esperando o segundo aperto
let lastShot=-99;
let gunKick=0;
let meleeAnim: MeleeAnim|null=null,punchSide=1;
...
const byIdSafe=(id: string)=>WEAPONS.find(w=>w.id===id)||FIST;
...
export function weaponAmmoInfo(id: string){
  const w=WEAPONS.find(x=>x.id===id);
  if(!w)return null;
  return{
    infinite:w.infiniteAmmo,
...
export function refillAmmo(id: string){
  const w=WEAPONS.find(x=>x.id===id);
...
export function pickupArsenalWeapon(id: string|undefined){
  const w=WEAPONS.find(x=>x.id===id);
...
function restoreInventory(list: unknown){
...
let trainingSnapshot: ArsenalSnapshot|null=null;
...
export function beginTrainingWeapon(id: string){
  const w=WEAPONS.find(x=>x.id===id);
  if(!w||w===FIST)return false;
  if(!trainingSnapshot){
...
export function snapshotArsenal(): ArsenalSnapshot{
  return{
    owned:owned.slice(),
    cur:curWeapon,
...
export function restoreArsenal(snap: ArsenalSnapshot|null){
...
function nearestWeaponPickup(maxD: number): THREE.Object3D|null{
  const pp=playerPos();
  let best: THREE.Object3D|null=null,bd=maxD;
...
function rayHitXZ(origin: THREE.Vector3,dir: THREE.Vector3,pos: THREE.Vector3,radius: number,range: number): number|null{
...
const _ray: Ray={origin:_muzzle,dir:_aimPoint};
...
function getMuzzleWorldPosition(out: THREE.Vector3): THREE.Vector3{
...
function syncHolderParent(obj: THREE.Object3D,toCamera: boolean,bodyScale: number,bodyReset: {pos: number[];rot: number[]}|null){
  if(toCamera){
    if(obj.parent!==camera)camera.add(obj);
  }else if(obj.parent!==player.g){
    player.g.add(obj);
    obj.scale.setScalar(bodyScale);
    if(bodyReset){obj.position.set(bodyReset.pos[0],bodyReset.pos[1],bodyReset.pos[2]);obj.rotation.set(bodyReset.rot[0],bodyReset.rot[1],bodyReset.rot[2]);}
  }
}
...
function mixArr(a: number[],b: number[],t: number){return a.map((v,i)=>lerp(v,b[i],t));}
function setRot(o: THREE.Object3D|undefined,r: number[]){if(o)o.rotation.set(r[0],r[1],r[2]);}

function spawnMeleeTrail(kind: string,side: number){
  const bat=kind==='bat';
...
function startMeleeAnimation(range: number|undefined,knock: number|undefined,lethal: boolean|undefined){
...
function tallyPunch(target: any,threshold: number){
...
function resolveMeleeImpact(a: MeleeAnim){
...
function applyPunchPose(a: MeleeAnim,p: number){
...
function applyBatPose(a: MeleeAnim,p: number){
  const l=player.g.userData.limbs;if(!l)return;
...
function updateMeleeAnimation(dt: number){
...
function updateMeleeTrails(dt: number){
  for(let i=meleeTrails.length-1;i>=0;i--){
    const s=meleeTrails[i];s.t+=dt;
    const mat=s.m.material as THREE.Material&{opacity: number};
    mat.opacity=Math.max(0,mat.opacity*(1-dt*5));
    s.m.scale.multiplyScalar(1+dt*1.6);
    if(s.t>=s.life){
      s.m.parent?.remove(s.m);
      s.m.geometry.dispose();(s.m.material as THREE.Material).dispose();
...
function blastDamage(pos: THREE.Vector3,opts?: {noSelf?: boolean}){
...
export function explodeAt(pos: THREE.Vector3,opts?: {noSelf?: boolean}){
  makeExplosion(pos.clone());
  blastDamage(pos,opts);
}
refs.explodeAt=explodeAt;

function explodeCar(car: any,arr: any[]){
...
function damageCar(car: any,arr: any[],pos: THREE.Vector3|null,dir: THREE.Vector3|null,dmg=1){
  if(!car||car===cur)return;
...
function addTracer(origin: THREE.Vector3,end: THREE.Vector3){
...
function addImpact(pos: THREE.Vector3,hit: WeaponHit|{kind: string}|null){
...
function missileBlast(pos: THREE.Vector3,hit: WeaponHit|null){
...
function flameAttack(range: number){
  const{origin,dir}=aimRay(range);
...
function throwProjectile(kind: string|undefined,{power=16,fuse}: {power?: number;fuse?: number}={}){
  ...
  thrown.push({g:mesh,vel,kind:kind!,fuse:fuse!=null?fuse:0,
...
function addFirePool(pos: THREE.Vector3){
...
const api: WeaponApi={
  ...
  gunshot(v: number){
    // The player's PISTOL fires the recorded pistol-shot sample; every other
  ...
  flame(range: number){flameAttack(range);},
  throwProjectile(kind: string|undefined,opts: {power: number;fuse?: number}){throwProjectile(kind,opts);},
...
export function updateWeapons(dt: number){
  ...
  for(let i=explosions.length-1;i>=0;i--){
    ...
    const s=1+e.t*4;
    e.g.scale.set(s,s,s);
    e.g.traverse(o=>{const m=(o as THREE.Mesh).material as THREE.Material&{opacity: number};if(m)m.opacity=Math.max(0,m.opacity-dt*1.6);});
    if(e.t>.75){disposeGeometries(e.g);scene.remove(e.g);explosions.splice(i,1);}
  }
  for(let i=tracers.length-1;i>=0;i--){
    const t=tracers[i];t.t+=dt;
    ((t.line as THREE.Line).material as THREE.Material&{opacity: number}).opacity=Math.max(0,1-t.t*8);
  ...
  for(let i=impacts.length-1;i>=0;i--){
    ...
    ((p.ring as THREE.Mesh).material as THREE.Material&{opacity: number}).opacity=Math.max(0,.85-p.t*4);
  ...
  for(let i=flameJets.length-1;i>=0;i--){
    ...
    f.g.traverse(o=>{const m=(o as THREE.Mesh).material as THREE.Material&{opacity: number};if(m&&m.opacity!=null)
      m.opacity=Math.max(0,m.opacity-dt*4.5);});

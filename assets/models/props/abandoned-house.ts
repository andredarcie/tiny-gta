...
const matFor=(map: Map<number,THREE.MeshLambertMaterial>,c: number): THREE.MeshLambertMaterial=>{if(!map.has(c))map.set(c,matte({color:c,roughness:1}));return map.get(c)!;};
...
const mk=(w: number,h: number,d: number,mat: THREE.Material,x: number,y: number,z: number,rx?: number,ry?: number,rz?: number): THREE.Mesh=>{
...
function boardedWindow(g: THREE.Group,x: number,y: number,z: number): void{
...
export function addAbandonedHouse(cx: number,cz: number,ry: number): {x0:number;x1:number;z0:number;z1:number;h:number}{
  const g=build();

...
export class GameDriver {
  ...
  async boot(nick = 'BOT') {
    ...
    await page.evaluate((n) => { document.getElementById('nick-input').value = n; }, nick);
  ...
  async waitForState(pred, { timeout = 20_000, message } = {}) {
    try {
  ...
  // ---- input (real keyboard) ---------------------------------------------
  async down(key) { if (!this._held.has(key)) { this._held.add(key); await this.page.keyboard.down(key); } }
  async up(key) { if (this._held.has(key)) { this._held.delete(key); await this.page.keyboard.up(key); } }
  async tap(key) { await this.page.keyboard.press(key); }
  ...
  async inPage(fn, arg) { return this.page.evaluate(fn, arg); }
  ...
  async enterCar() {
    await this.inPage(() => window.__test.enterCar());
  ...
  async placeVehicle(x, z, faceX, faceZ) {
    const ok = await this.inPage((a) => window.__test.placeVehicle(a.x, a.z, a.fx, a.fz),
  ...
  async raceFrame() {
    return this.inPage(() => ({ s: JSON.parse(window.render_game_to_text()), t: window.__test.raceTarget() }));
  }
  ...
  async startRaceByKey(stateKey, { tries = 4 } = {}) {
    await this.page.bringToFront().catch(() => {});
    await this.inPage(() => { const a = document.activeElement; if (a && a.blur) a.blur(); });
    const racing = (k) => {
      const s = JSON.parse(window.render_game_to_text());
    ...
    if (!started) {
      await this.inPage(() => window.__test.interact());    // focus fallback (still starts the race)
  ...
  async driveRace(stateKey, { maxMs = 120_000, tickMs = 70, sample = false, botch = [], botchTicks = 7 } = {}) {
    const page = this.page;
    await page.waitForFunction(
      (k) => { const s = JSON.parse(window.render_game_to_text()); return s[k] && s[k].phase === 'racing'; },
      stateKey, { timeout: 20_000 });
    const moneyStart = (await this.snapshot()).money;

    await page.keyboard.down('w');                          // accelerate
    let steer = null, stuck = 0, reversing = 0, place = null, cp = 0, finished = false;
    let raceTimeS = 0; const samples = []; const fumbled = new Set(); let fumbleTicks = 0;
    const setSteer = async (k) => { if (k !== steer) { if (steer) await page.keyboard.up(steer); if (k) await page.keyboard.down(k); steer = k; } };
    const t0 = Date.now();

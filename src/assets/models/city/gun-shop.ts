...
export const GUN_RANGE_TARGETS:any[]=[]; // {x,z,r,g,hit(),hitT}

export const gunShopFx:any={keeper:null,exitArrow:null,facade:null,facadeArrow:null,
...
function labelTexture(name:string,price:number):THREE.CanvasTexture{
...
function makeLabel(name:string,price:number):THREE.Mesh{
...
function panelTexture(lines:string,{bg='#17130f',fg='#ffe7b0',sub='#f5c518'}:{bg?:string,fg?:string,sub?:string}={}):THREE.CanvasTexture{
...
function makePanel(lines:string,w:number,h:number,opts?:{bg?:string,fg?:string,sub?:string}):THREE.Mesh{
...
function displayWeapon(w:any,x:number,topY:number,z:number):THREE.Group{
  ...
  model.traverse((o:THREE.Object3D)=>{o.castShadow=false;});
  const pivot=new THREE.Group();
  pivot.position.set(x,topY,z);
  pivot.add(model);
  return pivot;
}
function wallWeapon(w:any,x:number,y:number,z:number,rotY=Math.PI/2,scale=.92):void{
  const model=w.makeModel();
  model.updateMatrixWorld(true);
  let box=new THREE.Box3().setFromObject(model);
  const size=new THREE.Vector3();box.getSize(size);
...
function addDisplayCase(z:number):void{
...
function makeAmmoBox(label:string,color=0x6b4b26):THREE.Group{
  const g=new THREE.Group();
...
function addAmmoShelf(x:number,z:number,rotY:number,label:string):void{
...
function addSecurityCamera(x:number,y:number,z:number,rotY:number):void{
...
function addWallBox(x:number,y:number,z:number,w:number,h:number,d:number,mat:THREE.Material=wallM,group:THREE.Group=gunShopInterior):THREE.Mesh{
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);
  m.position.set(x,y,z);m.receiveShadow=true;m.castShadow=true;
  group.add(m);
  return m;
}
function addRangeTarget(x:number,z:number,lane:number,group:THREE.Group=gunRangeInterior):any{
  const tex=canvasTexture(256,256,(c,w,h)=>{
    ...
    for(const[r,col]of[[88,'#161616'],[64,'#f3f3f0'],[42,'#161616'],[20,'#d02525']] as [number,string][]){
  ...
  group.add(g);
  const target={x,z,r:.82,g,hitT:0,hit(this:{hitT:number}){
    this.hitT=.22;
    (g.material as THREE.MeshBasicMaterial).color.setHex(0xffd24a);
  }};
  GUN_RANGE_TARGETS.push(target);
  gunShopFx.rangeTargets.push(target);
  return target;
}
function floorWeapon(w:any,x:number,z:number,rotY=0,group:THREE.Group=gunRangeInterior):void{
  const model=w.makeModel();
  model.updateMatrixWorld(true);
  let box=new THREE.Box3().setFromObject(model);
  const size=new THREE.Vector3();box.getSize(size);
  ...
  model.rotation.x=.06;model.traverse((o:THREE.Object3D)=>{o.castShadow=false;});
  const pivot=new THREE.Group();
...
function addLaneNumber(n:number,x:number,z:number,group:THREE.Group=gunRangeInterior):void{
  const p=makePanel(String(n),.72,.52,{bg:'#111',fg:'#fff6d0',sub:'#f5c518'});
  p.position.set(x,3.55,z);p.rotation.y=-Math.PI/2;
  group.add(p);
}
function addShootingRange(solids:{x0:number,x1:number,z0:number,z1:number,h:number}[]):void{
  ...
  ARSENAL.forEach((w:any,i:number)=>floorWeapon(w,xs[i%4],zs[Math.floor(i/4)],i*.55,group));
...
export function addGunShop(solids:{x0:number,x1:number,z0:number,z1:number,h:number}[]):void{
  ...
  const rackWeapons=ARSENAL.filter((w:any)=>['pistol','uzi','shotgun','ak47','m16','sniper','rocket','flame'].includes(w.id));
  rackWeapons.slice(0,4).forEach((w:any,i:number)=>wallWeapon(w,-808+i*2.1,3.15,-390.45,Math.PI/2,.95));
  rackWeapons.slice(4).forEach((w:any,i:number)=>wallWeapon(w,-785.52,3.1,-388+i*2.15,0,.95));
  ...
  ARSENAL.forEach((w:any,i:number)=>{

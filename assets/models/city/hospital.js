...
function poster(title,sub,bg,fg,W=256,H=160){
...
const eyeChartM=new THREE.MeshBasicMaterial({map:tex(180,256,(x,w,h)=>{
  ...
  const rows=[['E',58],['F P',42],['T O Z',30],['L P E D',22],['F E C F D',16],['E D F C Z P',12]];
  let y=36;
...
function wallPoster(mat,w,h,x,y,z,ry){
...
export function addHospital(solids){
  ...
  const place=(mesh,x,y,z)=>{mesh.position.set(x,y,z);hospInterior.add(mesh);};

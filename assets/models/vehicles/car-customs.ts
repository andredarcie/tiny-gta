...
export function paintMat(color: number): THREE.MeshStandardMaterial{
...
const hubCache=new Map<number,THREE.MeshStandardMaterial>();
function hubMat(color: number): THREE.MeshStandardMaterial{
...
function mods(carG: THREE.Object3D){return carG.userData.mods||(carG.userData.mods={
  paint:carG.userData.color??0xff2e88,rims:'stock',spoiler:'none',neon:null,hood:'stock'});}

// ---------- pintura ----------
function paintMeshes(carG: THREE.Object3D): THREE.Mesh[]{
...
export function applyPaint(carG: THREE.Object3D,color: number): void{
  ...
  if(sp&&sp.userData.bodyColored)sp.traverse((o: any)=>{if(o.userData.paintPart)o.material=m;});
}
...
// ---------- aerofólio ----------
function buildSpoiler(type: string,paint: THREE.Material): THREE.Group{
...
// ---------- capô (scoop / vents) ----------
function buildHood(type: string): THREE.Group{
  const g=new THREE.Group();
  if(type==='scoop'){
...
export function repairCar(carG: THREE.Object3D): boolean{
  let fixed=false;
...
function buildDemo(): THREE.Group{
  const g=new THREE.Group();

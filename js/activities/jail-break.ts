...
const near=(p: {x:number;z:number},t: {x:number;z:number},r: number)=>Math.hypot(p.x-t.x,p.z-t.z)<r;

// Auto-equipped tunnel flashlight (lazily created, parked off until you're inside).
let flash: THREE.Object3D|null=null; const _hand=new THREE.Vector3(),_dir=new THREE.Vector3();
...
export function updateJailBreak(dt: number): void{
  if(cd>0)cd-=dt;
  // glow pulses (cheap, always)
  const t=state.time;
  if(tunnelFx.buttonGlow)(tunnelFx.buttonGlow.material as THREE.Material).opacity=.6+.3*Math.sin(t*3);
  if(tunnelFx.gateGlow)(tunnelFx.gateGlow.material as THREE.Material).opacity=.6+.3*Math.sin(t*4);
  if(tunnelFx.bulbs)for(const b of tunnelFx.bulbs)            // gentle bulb/lantern flicker
    (b.m.material as THREE.Material).opacity=b.base-.18*Math.abs(Math.sin(t*b.sp+b.ph)*Math.sin(t*b.sp*.37));
...
function arrive(fn: ()=>void,msg: string,colorOrBlip: string|number[],maybeBlip?: number[]): void{
...
function teleportWorld(x: number,z: number,h: number): void{

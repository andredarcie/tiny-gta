...
function keepOutTex(): THREE.CanvasTexture {
...
function build(): THREE.Group {
  ...
  const seg=(x0: number,z0: number,x1: number,z1: number,h=WALL_H): void=>{
  ...
  const bag=(x: number,y: number,z: number): void=>{
...
export function addAbandonedFort(solids: {x0:number;x1:number;z0:number;z1:number;h:number}[],cx: number,cz: number): void {

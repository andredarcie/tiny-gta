...
function airfoil(chord: number, thick: number): THREE.Shape{
...
function makeWing(span: number, chord: number, thick: number, mat: THREE.Material): THREE.Group{
...
function strut(a: THREE.Vector3, b: THREE.Vector3, r: number, mat: THREE.Material, shadow = true): THREE.Mesh{
...
function makeWheel(radius: number, tube: number, hubM: THREE.Material, tyreM: THREE.Material): THREE.Group{
...
export const makePlane = (): THREE.Group => { const g = build(); scene.add(g); return g; };

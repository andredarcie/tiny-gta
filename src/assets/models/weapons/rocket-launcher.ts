...
export function makeRocketLauncherModel({pickup=false}:{pickup?:boolean}={}): THREE.Group{
...
// Míssil/foguete: corpo metálico, ogiva vermelha e chama do motor (tremula no voo)
export function makeMissileModel(): THREE.Group{
  const g=new THREE.Group();
...
export default {category:'Weapons',label:'Rocket Launcher',build:()=>makeRocketLauncherModel(),
  variants:[{label:'Rocket Launcher',build:()=>makeRocketLauncherModel()},
    {label:'Missile',build:()=>makeMissileModel()}]};

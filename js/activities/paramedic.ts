...
const amb: any={g:makeAmbulance(),heading:0,speed:0,name:'AMBULANCE',police:false,ambulance:true};
...
class Patient extends Npc{
  x:number;z:number;ring:THREE.Mesh|null;beacon:THREE.Mesh|null;loaded:boolean;
  constructor(g:THREE.Object3D,x:number,z:number,ring:THREE.Mesh,beacon:THREE.Mesh){
...
const patients: Patient[]=[]; // {g,x,z,ring,beacon,loaded}
let hospMk: Marker|null=null;   // marcador do hospital quando todos a bordo
...
refs.isAmbulanceCar=(c: any)=>c===amb;
refs.paramedicBlips=()=>{
  if(phase==='off')return[];
...
export function updateParamedic(dt: number){

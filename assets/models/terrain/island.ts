...
function layFlat(geo: THREE.BufferGeometry,mat: THREE.Material,y: number): THREE.Mesh{
...
function cityShape(extra=0): THREE.Shape{
...
function cityRing(rOut: number,thick: number): THREE.ShapeGeometry{
...
function cityAnnulus(inExtra: number,outExtra: number): THREE.ShapeGeometry{
  const sh=new THREE.Shape(),hole=new THREE.Path();
  for(let i=0;i<=NA;i++){
...
function ruralEdgeStrip(rOut: number,thick: number,sign: number): THREE.ShapeGeometry{
...
export function updateCoastFoam(foam: Foam[],time: number): void{
  for(const w of foam){
    if(w.polar){
      const s=1+w.amp*(.5+.5*Math.sin(time*w.spd+w.ph));
      w.m.scale.set(s,s,1);
    }
    (w.m.material as THREE.Material).opacity=(w.polar?.06:.05)+.28*Math.max(0,Math.sin(time*w.spd+w.ph+1.2));
  }
}

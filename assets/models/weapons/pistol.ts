...
const box=(w:number,h:number,d:number,mat:THREE.Material,x:number,y:number,z:number,rx=0,ry=0,rz=0,cast=false): THREE.Mesh=>{
  const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat);
  m.position.set(x,y,z);m.rotation.set(rx,ry,rz);m.castShadow=cast;
  return m;
};
const cyl=(r:number,len:number,mat:THREE.Material,x:number,y:number,z:number,rx=0,cast=false): THREE.Mesh=>{
  const m=new THREE.Mesh(new THREE.CylinderGeometry(r,r,len,12),mat);
  m.position.set(x,y,z);m.rotation.x=rx;m.castShadow=cast;
  return m;
};

export function makePistolModel({pickup=false}:{pickup?:boolean}={}): THREE.Group{

...
function parabolicArch(w: number,hRect: number,rise: number): THREE.Shape {
...
function lancetWindow(w: number,h: number): THREE.Group {
...
const band=(cx: number,cy: number,w: number,th: number,z: number,d=.2,m: THREE.Material=moldM): THREE.Mesh=>{const b=new THREE.Mesh(new THREE.BoxGeometry(w,th,d),m);b.position.set(cx,cy,z);return b;};
...
function build(): THREE.Group {
  ...
  const tri=(w: number,r: number): THREE.Shape=>{const sh=new THREE.Shape();sh.moveTo(-w/2,0);sh.lineTo(w/2,0);sh.lineTo(0,r);sh.closePath();return sh;};
...
export function addIgrejaDivino(cx: number,cz: number,ry=0): {x0:number;x1:number;z0:number;z1:number;h:number} {

...
import type { LedgerTx, LedgerSnapshot, EconomyDebug } from './types.js';
...
function clean(amount: unknown): number {
...
class Economy{
  earned=0; spent=0;                  // running session totals (debug/stats)
  ledger: LedgerTx[]=[];              // recent txs {id,amt,why,t,local}
  seen=new Set<string>();             // ids currently in `ledger` (O(1) dedupe)
  checkpoint=0;                       // folded sum of compacted/imported txs
  sum=0;                              // Σ amt of the txs currently in `ledger`
  ...
  get balance(): number {return state.money;}
  ...
  _recompute(): void { state.money=Math.max(0,this.checkpoint+this.sum); }
  ...
  earn(amount: number, source='', {persist=true,id}: {persist?: boolean; id?: string}={}): number {
  ...
  canAfford(amount: number): boolean {return state.money>=clean(amount);}
  ...
  spend(amount: number, reason=''): boolean {
    const amt=clean(amount);
  ...
  penalty(keepFraction: number, reason=''): number {
  ...
  importLedger(saved: LedgerSnapshot | null): void {
  ...
  _sanitize(raw: unknown): LedgerTx | null {
    if(!raw||typeof raw!=='object')return null;
    const r=raw as Record<string, unknown>;
    const id=typeof r.id==='string'&&ID_OK.test(r.id)?r.id:null;
  ...
  history(): { amt: number; why: string; t: number }[] {
    const out: { amt: number; why: string; t: number }[]=[];
  ...
  stats(): { balance: number; earned: number; spent: number; count: number } {

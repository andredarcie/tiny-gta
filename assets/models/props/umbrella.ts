...
const topMats=new Map<number,THREE.MeshLambertMaterial>(),towelMats=new Map<number,THREE.MeshLambertMaterial>();
const topFor=(c: number): THREE.MeshLambertMaterial=>{if(!topMats.has(c))topMats.set(c,
  matte({color:c,roughness:.85,side:THREE.DoubleSide}));
  return topMats.get(c)!;};
const towelFor=(c: number): THREE.MeshLambertMaterial=>{if(!towelMats.has(c))towelMats.set(c,
...
export function addUmbrella(x0: number,z0: number): THREE.Group{
  const g=build();

...
**Engine.** `src/js/core/engine.ts` owns the renderer, scene, camera, lights, fog, and mobile pixel-ratio/shadow limits. `src/js/core/constants.ts` holds the world-grid math (`N`,`CELL`,`ROAD`,`HALF`…) and `groundHeight(x,z)` — terrain collision interpolates the *same* triangles the visual mesh uses, so physics matches what's drawn.

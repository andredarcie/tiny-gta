...
const nearCorner=s=>coastCorners.some(c=>Math.abs(s-c)<CORNER_MARGIN);
...
startMk.ring.rotation.x=Math.PI/2;startMk.ring.material.transparent=true;
...
let freezePos=null; // posição travada da lancha durante a contagem
...
function spawnRacers(h0){
  ...
  for(let i=0;i<NPC_COUNT;i++){
    const g=makeBoat(boatColors[i%boatColors.length]); // makeBoat já adiciona à cena
...
function updateCpMarkers(dt){
  for(let i=0;i<cpMarkers.length;i++){
    ...
    if(m.beacon){m.beacon.visible=show;if(show)m.beacon.material.opacity=current?.18:.08;}
    if(m.finish){
...
const rwake=[],rwakePool=[];
function spawnRivalPuff(x,z,s0,s1,life){

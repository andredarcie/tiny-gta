...
const fmtMoney=n=>'$'+Math.abs(Math.floor(Number(n)||0)).toLocaleString('en-US');
...
// ---- shared chrome ---------------------------------------------------------
const backBtn=()=>`<button class="pause-btn pause-back" data-act="back">&#8592; BACK</button>`;
function pager(page,pages,note){
...
let lbEntries=[],lbTotal=0,lbPage=0,lbReqId=0;
...
const findItem=key=>{for(const g of SCHEMA)for(const it of g.items)if(it.key===key)return it;return null;};
...
function onBodyClick(e){
  ...
  const sw=e.target.closest('[data-set][data-type="toggle"]');
  if(sw){
    e.stopPropagation();
    const key=sw.dataset.set,nv=!settings[key];
    setSetting(key,nv);
...
function onBodyInput(e){
  const r=e.target.closest('[data-set][data-type="range"]');
  if(!r)return;
